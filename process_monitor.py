...
def get_processes():
    ...
    for proc in psutil.process_iter(attrs=['pid', 'name', 'cpu_percent', 'memory_info', 'username', 'nice']):
        try:
            ...
            info['memory_info'] = info['memory_info'].rss // 1024**2 if info['memory_info'] else 0  # Convert to MB
...
def update_process_list():
    ...
    check_cpu_memory_alerts()  # Check alerts
    root.after(3000, update_process_list)  # Schedule next update
...
# Function to change priority of a process
...
# Function to update CPU & Memory usage graph
def update_graph():
    ...
    canvas.draw()
    root.after(2000, update_graph)  # Schedule next update

# Function to check CPU & Memory usage alerts
